  ...
  subroutine ELMFatesGlobals2()
   ...
   subroutine dynamics_driv(this, bounds_clump, top_as_inst,          &
      ...
      do s=1,this%fates(nc)%nsites
         ...
         nlevsoil = this%fates(nc)%bc_in(s)%nlevsoil ! GML Keep the levsoil the same for now

         ! Decomposition fluxes

         do ifp = 1, this%fates(nc)%sites(s)%youngest_patch%patchno
            ifc = f2hmap%GetColumnIndex(ifp)
            this%fates(nc)%bc_in(s)%w_scalar_sisl(ifp,1:nlevsoil) = col_cf%w_scalar(ifc,1:nlevsoil)
            this%fates(nc)%bc_in(s)%t_scalar_sisl(ifp,1:nlevsoil) = col_cf%t_scalar(ifc,1:nlevsoil)
         end do
